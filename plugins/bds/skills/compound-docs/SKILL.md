---
name: compound-docs-python
description: Capture solved problems as categorized documentation with YAML frontmatter for fast lookup (Python projects)
allowed-tools:
...
# compound-docs-python Skill

**Purpose:** Automatically document solved problems to build searchable institutional knowledge with category-based organization (enum-validated problem types) for Python projects.

## Overview

This skill captures problem solutions immediately after confirmation, creating structured documentation that serves as a searchable knowledge base for future sessions.

**Organization:** Single-file architecture - each problem documented as one markdown file in its symptom category directory (e.g., `docs/solutions/performance-issues/sqlalchemy-n-plus-one.md`). Files use YAML frontmatter for metadata and searchability.
...
- **Module name**: Which module/package had the problem
- **Symptom**: Observable error/behavior (exact error messages, tracebacks)
...
- Python version
- Framework version (Django, FastAPI, Flask, etc.)
...
2. What was the exact error message or traceback?
3. What framework/library versions are you using?
...
- `import-error-database-module-20251130.md`
- `sqlalchemy-n-plus-one-user-service-20251130.md`
- `async-timeout-api-client-20251130.md`
</step>
...
- severity: must be one of [critical, high, medium, low], got "blocker"
...
- Foundational requirement (async patterns, type hints, testing, etc.)
...
Action:
1. Prompt: "What should the new skill be called? (e.g., sqlalchemy-patterns, fastapi-testing)"
2. Create skill structure with this solution as first example
...
- Extract exact error messages and tracebacks from conversation
...
- ✅ Exact error messages and tracebacks (copy-paste from output)
...
2. **Gather context:**
   - Module: User Service
   - Symptom: User list endpoint taking >5 seconds, SQLAlchemy N+1 query when loading user profiles
   - Failed attempts: Added pagination (didn't help), checked database indexes
   - Solution: Added eager loading with `joinedload(User.profile)` on query
   - Root cause: Missing eager loading causing separate database query per user
3. **Check existing:** No similar issue found
4. **Generate filename:** `n-plus-one-user-list-user-service-20251130.md`
5. **Validate YAML:**
   ```yaml
   module: User Service
   date: 2025-11-30
   ...
   symptoms:
     ...
     - "User list endpoint taking >5 seconds"
...
6. **Create documentation:**
   - `docs/solutions/performance-issues/n-plus-one-user-list-user-service-20251130.md`
